...
Each loader is idempotent, so re-running is safe. The CO2e/€ metric in
`export_json.py` needs *both* dividends and emissions loaded; with only one it
stays `null` and prints a warning. Don't break this! Emissions are read from `dev/climate_data/cdu_cac40_cleaned.csv`
by `load_emissions.py` 
